  ...
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match self {
      ...
      Self::ParseFailed(msg) => {
        write!(f, "Failed to parse Tailscale status: {msg}")
      },
      Self::MissingData(msg) => {
        write!(f, "Missing data in Tailscale status: {msg}")
      },
    }
  }
}
...
pub fn get_current() -> Result<Context, AppError> {
  let status = get()?;

  if status.this_machine.ips.is_empty() {
    error!("This machine has no IP addresses");
    return Err(AppError::from(StatusError::MissingData(
...
pub fn get_json() -> Result<String, AppError> {
  let output = Command::new("tailscale")
    ...
    .map_err(|e| {
      ...
      AppError::from(StatusError::CommandFailed(e.to_string()))
    })?;

  if !output.status.success() {
    let stderr = String::from_utf8_lossy(&output.stderr);
    error!("Tailscale status command failed: {}", stderr.trim());
    return Err(AppError::from(StatusError::CommandFailed(format!(
  ...
  let stdout = String::from_utf8(output.stdout).map_err(|e| {
    ...
    AppError::from(StatusError::ParseFailed(format!(
